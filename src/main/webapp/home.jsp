...
<section class="cta-wrap">
    <div class="cta-card">
        ...
        <p>
            Share your favorite local spots with fellow explorers and help others
            discover the best of Terengganu.
        </p>
            ...
            <div class="treasures-grid">
                <%
                    try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/ltwbs", "root", "")) {
                        ...
                        // Pastikan business_id IS NULL supaya akaun kedai business tidak bertindih masuk seksyen lokasi awam
                        whereList.add("l.business_id IS NULL");

                        if (selectedCategoryId > 0) {
                            whereList.add("l.category_id = ?");
                        }
                        if ("featured".equals(filter)) {
                            whereList.add("l.is_featured = 1");
                        }
                        ...
                        if ("rated".equals(filter)) {
                            sql.append("ORDER BY avg_rating DESC, l.location_id DESC ");
                        } else if ("views".equals(filter)) {
                            sql.append("ORDER BY l.views DESC, l.location_id DESC ");
                        } else if ("popular".equals(filter)) {
                            sql.append("ORDER BY total_reviews DESC, l.views DESC, l.location_id DESC ");
                        } else {
                            sql.append("ORDER BY l.location_id DESC ");
                        }
                        ...
                        try (PreparedStatement ps = conn.prepareStatement(sql.toString())) {
                            ...
                            if (selectedCategoryId > 0) {
                                ps.setInt(idx++, selectedCategoryId);
                            }
                            if (searchText != null && !searchText.trim().isEmpty()) {
                            ...
                            try (ResultSet rs = ps.executeQuery()) {
                                ...
                                while (rs.next()) {
                                    ...
                                    String imgUrl = (img != null && !img.trim().isEmpty()) ? request.getContextPath() + "/LocationImageServlet?file=" + URLEncoder.encode(img.trim(), "UTF-8") : "image/placeholder.jpg";
                %>
                    ...
                    <div class="card-content">
                        ...
                        <div style="margin-top:10px;display:flex;gap:10px;align-items:center;">
                            ...
                            <form action="ToggleBookmarkServlet" method="post" style="display:inline;">
                                ...
                                <input type="hidden" name="redirect" value="home.jsp?categoryId=<%= selectedCategoryId%>&filter=<%= filter%>&search=<%= URLEncoder.encode(searchText, "UTF-8")%>#loc-<%= locId%>">
...
<section class="treasures-section container">
    ...
    <div class="treasures-grid">
        <%
            ...
            try {
                ...
                // Query SQL ini menghubungkan jadual businesses ke jadual location, 
                // kemudian mengira rating dari jadual 'reviews' asal berdasarkan location_id tersebut.
                String homeBizSql = "SELECT b.business_id, b.user_id, b.business_name, b.description, b.address, " +
                ...
                homeBizPs = homeBizConn.prepareStatement(homeBizSql);
                ...
                while (homeBizRs.next()) {
                    ...
                    String bImgUrl = request.getContextPath() + "/image/placeholder.jpg";
                    if (bImg != null && !bImg.trim().isEmpty()) {
                        bImgUrl = request.getContextPath() + "/uploads/" + java.net.URLEncoder.encode(bImg.trim(), "UTF-8");
                    } else {
                        bImgUrl = request.getContextPath() + "/uploads/default_business.jpg";
                    }
        ...
        %>
                <div class="treasure-card">
                    <img src="<%= bImgUrl %>" alt="<%= bName %>" class="card-image" style="height:180px; object-fit:cover;">
                    <div class="card-content">
  ...
  <a class="card-btn"
   href="<%= request.getContextPath() %>/businessReviews?id=<%= bId %>"
   style="display:inline-block; text-align:center; flex:1; background:#111; color:#fff; text-decoration:none;">
...
<footer class="footer">
    ...
    function searchLocations() {
        ...
        let url = "home.jsp?filter=" + encodeURIComponent(filter) + "&categoryId=" + currentCategoryId;
        if (searchText.trim() !== "") {
