...
<%
    ...
    if (bPhone != null && !bPhone.trim().isEmpty()) {
        ...
        if (cleanPhone.startsWith("0")) {
            cleanPhone = "6" + cleanPhone; // Convert Malaysia 01x to 601x
        }
    ...
    if (bImg != null && !bImg.trim().isEmpty()) {
        if (bImg.startsWith("sub_")) {
            imgUrl = request.getContextPath() + "/LocationImageServlet?file=" + URLEncoder.encode(bImg.trim(), "UTF-8");
        } else if (bImg.startsWith("business_")) {
            imgUrl = request.getContextPath() + "/uploads/" + URLEncoder.encode(bImg.trim(), "UTF-8");
        } else {
...
<head>
    ...
    <style>
        ...
        .promo-card { background: #fff; border: 2px dashed #f59e0b; border-radius: 16px; padding: 20px; margin-bottom: 15px; position: relative; overflow: hidden; }
        ...
        <div class="biz-info">
            ...
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <div><%= bAddr != null ? bAddr : "-" %></div>
                </div>
                <div class="info-item">
                    <i class="fas fa-clock"></i>
                    <div><%= bHours != null ? bHours : "-" %></div>
                </div>
                <div class="info-item">
                    <i class="fas fa-phone"></i>
                    <div><%= bPhone != null ? bPhone : "-" %></div>
                </div>
            </div>
            ...
            <div class="action-btns">
                ...
                <% } %>
                <a href="<%= gmapsLink %>" target="_blank" class="btn-map">
                    <i class="fas fa-map-marker-alt"></i> Show in Maps
                </a>
                <a href="<%= request.getContextPath() %>/businessReviews?id=<%= bId %>" class="btn-reviews">
            ...
            <% for (Map<String,Object> promo : promotionsList) { %>
                <div class="promo-card">
                    ...
                    <div class="promo-date">
                        <i class="fas fa-calendar-alt"></i> Valid: <%= promo.get("start") %> to <%= promo.get("end") %>
                    </div>
                </div>
            <% } %>
        <% } else { %>
            <div style="background:#fff; padding:20px; border-radius:12px; border:1px solid #eee; color:#6b7280; text-align:center; font-weight:600;">
